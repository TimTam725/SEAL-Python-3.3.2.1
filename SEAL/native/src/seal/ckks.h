    ...
    */
    ...
    private:
                ...
                MemoryPoolHandle pool)
        {
            ...
            {//13 bitの写像
                conj_values[matrix_reps_index_map_[i]] = values[i];//0, 4096, 1024, 5632, 640, 5056, 1744, 5188, 358
                conj_values[matrix_reps_index_map_[i + slots_]] = std::conj(values[i]);//8192, 4095, 7167
            }
            ...
            for (int i = 0; i < logn; i++)
            {
                ...
                for (std::size_t j = 0; j < h; j++)
                {
                    ...
                    }//heavy gentleman-sande バタフライ gs
            ...
            if (max_coeff_bit_count >= context_data.total_coeff_modulus_bit_count())//140
            {
